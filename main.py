...
def add_entry():
    ...
    if int(difficulty_entry.get()) < 1 or int(difficulty_entry.get()) > 5:
        messagebox.showerror("Error", 'The \'Difficulty\' must be between 1 and 5!')
...
frame = tk.Frame(canvas, pady=10)

canvas.create_window((0, 0), window=frame, anchor='nw')
frame.bind('<Configure>', lambda e: canvas.configure(scrollregion=canvas.bbox("all")))

task_name = tk.Label(frame, text='Task name', padx=5)
task_name.grid(column=0, row=0)
difficulty = tk.Label(frame, text='Difficulty', padx=5)
